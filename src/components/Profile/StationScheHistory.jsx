...
export default function StationScheduleHistory() {
  ...
  // Các hàm helper để lấy thông tin từ schedule - ĐÃ DI CHUYỂN LÊN TRÊN
  ...
  const fetchUserSchedules = async (accountId) => {
    try {
      ...
      if (schedulesData.length === 0) {
        setError('Không tìm thấy lịch sử đặt lịch. Có thể bạn chưa có lịch đặt nào.');
      }
  ...
  const filteredSchedules = React.useMemo(() => {
    ...
    return schedules.filter(schedule => {
      ...
      if (filterStatus === 'pending') {
        return status === 'pending' || status === 'confirmed' || status === 'approved' || status === 'active';
      }
  ...
  const sortedSchedules = React.useMemo(() => {
    if (!Array.isArray(filteredSchedules)) return [];
      ...
      <div className="container">
        ...
        <div style={{ 
          ...
          <button
            ...
            className={filterStatus === 'all' ? 'btn-primary' : 'btn-outline'}
            style={{
              background: filterStatus === 'all' ? 'linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%)' : 'transparent',
              color: filterStatus === 'all' ? 'white' : '#6366f1'
            }}
          >
            📋 Tất Cả
          </button>
          <button
            ...
            className={filterStatus === 'pending' ? 'btn-primary' : 'btn-outline'}
            style={{
              background: filterStatus === 'pending' ? 'linear-gradient(135deg, #f97316 0%, #ea580c 100%)' : 'transparent',
              color: filterStatus === 'pending' ? 'white' : '#f97316',
              borderColor: '#f97316'
            }}
          >
            ⏳ Đang Chờ
          ...
          <button
            ...
            className={filterStatus === 'completed' ? 'btn-primary' : 'btn-outline'}
            style={{
              background: filterStatus === 'completed' ? 'linear-gradient(135deg, #22c55e 0%, #16a34a 100%)' : 'transparent',
              color: filterStatus === 'completed' ? 'white' : '#22c55e',
              borderColor: '#22c55e'
            }}
          >
            ✅ Đã Hoàn Thành
        ...
        {sortedSchedules.length === 0 ? (
          <div className="empty-state">
            <h3>
              {filterStatus === 'pending' ? '⏳ Chưa có lịch đặt nào đang chờ' :
               filterStatus === 'completed' ? '✅ Chưa có lịch đặt nào đã hoàn thành' :
               '📝 Chưa có lịch đặt nào'}
            </h3>
            <p>
              {filterStatus === 'pending' ? 'Các lịch đặt đang chờ xác nhận sẽ được hiển thị tại đây' :
               filterStatus === 'completed' ? 'Các lịch đặt đã hoàn thành sẽ được hiển thị tại đây' :
          ...
          <>
            ...
            <div className="schedule-count">
              Hiển thị {sortedSchedules.length} lịch đặt
              {filterStatus === 'pending' && ' đang chờ'}
